 ...
 *  저장위치를 구분하여 명확하게 접근할 수있도록 해주는 것
...
public class run {
	 ...
	 *  
	 *  3. 객체지향 프로그래밍 : 현실세계릐 객체들간의 상호작요 -> 프로그래밍을 통해서 가상세계로 구현하는 가장 효과적인 방법
	 *  
	 ...
	 *  3) 추출한 것들을 가지고 내가 구현하고자하는 프로그램의 "실질적인 목적"에 맞춰 볼필요한 속성, 기능을 제거할 것
	 *  4) 최종적으로 추려진 속성들을 어떤 자료형으로, 어떤 변수명으로 사용할 것인지 정의
	 *  
	 *  
	 *  ex) 학생 관리 프로그램
	 *  1) 학생관련 객체(최경재훈련생, 최용준훈련생, 연재휘훈련생등등...)
	 *  2) 공통적인 속성및 기능(이름, 나이, 전화번호, 키, 몸무게, 머리색, 발사이즈, 달리기속도, 먹는다, 잔다, 화장실간다, 공부한다, 휴식한다)
	 *  3) 학생 "인적사항"을 관리하는 프로그램 : 이름, 나이, 주소, 전화전호, 반...
	 *     학생 "성적"을 관리하는 프로그램 : 이름, 반, 과목별 점수...
	 *     학생 "건강"을 관리하는 프로그램 : 이름, 반...
	 *  4) 이름 : String name;
	 *     나이 : int age;
	 *     키 : double height;
	 *  
	 ...
	 *        최강재라는 훈련생 객체 하나 만들기 -> String name1 = "최강재"; int age = 20; double height = 185.0;
	 *  	  안재휘라는 훈련생 객체 하나 만들기 -> String name1 = "안재휘"; int age = 19; double height = 193.0;
	 *  
	 *     2. 배열로 프로그래밍 한다면
	 ...
	 *         
	 *     3. 그래서 나온 개념 -> 구조체
	 *     구조체 : 여러개의 자료형의 여러개의 값들ㅇ르 보관할 수 있다.
	 ...
	 *     }
	 *     
	 *     -> 자바에서는 구조체 개념을 좀더 확장해서 클래스라는 개념을 만들었다.
	 *     
	 *     * 캡슐화 : 추상화를 통해서 정의된 속성들과 기능들을 하나로 묶어 관리하는 기법
	 *     			클래스에서 가장 중요한 목적인 "데이터의 접근제한(정보은닉)"을 원칙으로
	 *     			외부로부터의 "데이터 접근을 막고"
	 *     			대신에 "데이터를 간접적으로 처리(값을 변경하거나, 가져오는 것들)"할 메소드들을 클래스 내부에 작성하여 관리하는 것
	 *     			다양한 기능을 묶어 하나의 큰기능을 만드는것
	 */
